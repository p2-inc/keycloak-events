...
public class MdcLoggerEventListenerProviderTest {
  ...
  private EventListenerTransaction tx;
  ...
  public void setUp() {
    MDC.clear();
    provider = new MdcLoggerEventListenerProvider(fakeSession());
    tx = provider.getTransaction();
    eventLogger =
  ...
  @Test
  public void missingUserEventIdIsAutoGeneratedOnQueue() {
    Event event = new Event();
    ...
    // ID is set immediately when queued, not on commit
  ...
  @Test
  public void missingAdminEventIdIsAutoGeneratedOnQueue() {
    AdminEvent event = new AdminEvent();
  ...
  @Test
  public void onEventDoesNotLogUntilCommit() {
    Event event = new Event();
    event.setType(EventType.LOGIN);
    event.setRealmId("r");

    provider.onEvent(event);
    assertThat(eventAppender.list.size(), is(0));
  ...
  @Test
  public void onAdminEventDoesNotLogUntilCommit() {
    AdminEvent event = new AdminEvent();
    event.setOperationType(OperationType.CREATE);

    provider.onEvent(event, false);
    assertThat(adminAppender.list.size(), is(0));
  ...
  @Test
  public void rollbackDiscardsQueuedUserEvents() {
    Event event = new Event();
    event.setType(EventType.LOGIN);
  ...
  @Test
  public void rollbackDiscardsQueuedAdminEvents() {
    AdminEvent event = new AdminEvent();
    event.setOperationType(OperationType.CREATE);
  ...
  @Test
  public void constructorEnlistsTransactionAfterCompletion() {
    KeycloakTransaction[] enlisted = new KeycloakTransaction[1];
    KeycloakSession session = fakeSession(t -> enlisted[0] = t);

    MdcLoggerEventListenerProvider p = new MdcLoggerEventListenerProvider(session);

    assertThat(enlisted[0], is(notNullValue()));
    assertThat(enlisted[0], is((KeycloakTransaction) p.getTransaction()));
  }

  /** Fake session whose transaction manager silently captures enlistments. */
  private static KeycloakSession fakeSession() {
    return fakeSession(t -> {});
  }

  private static KeycloakSession fakeSession(
      java.util.function.Consumer<KeycloakTransaction> sink) {
    KeycloakTransactionManager tm =
        (KeycloakTransactionManager)
            Proxy.newProxyInstance(
                MdcLoggerEventListenerProviderTest.class.getClassLoader(),
                new Class<?>[] {KeycloakTransactionManager.class},
                (proxy, method, args) -> {
                  if ("enlistAfterCompletion".equals(method.getName())) {
                    KeycloakTransaction t = (KeycloakTransaction) args[0];
                    t.begin(); // mimic real KC behavior so commit/rollback are legal
                    sink.accept(t);
                    return null;
                  }
